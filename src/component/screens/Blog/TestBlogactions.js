...
var myfullname = "Admin";
var myproimage  = "http://servotech.in/wp-content/uploads/2016/10/user-icon-placeholder.png";

 var ref = firebase.database().ref("userInfo/" + currentUser.uid);

ref.once("value").then(function(snapshot) {
  var user = snapshot.val();
  myfullname = user.fullname;
  myproimage = user.profileImage;
    ...
    return (dispatch) => {
        ...
        this.multiuploadImage(imageArray).then(url  =>
         ...
         this.changedesc(desc, url).then(uploadDescription=> 


            firebase.database().ref('blogs/' + currentUser.uid).push({

                imageUrl: uploadDescription,
 ...
 var uploadDescription;
     var rgx = /<img src="([^">]+)"/g;
        var match; 
         ...
         match = rgx.exec(desc);
         uploadDescription =  desc.replace(match[1], url);
